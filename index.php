...
if($debug_mode){
	debugMessage("Using server url - <a href='$_server_url' target='_blank'>$_server_url</a>");
	//Get contents (showing errors)
	$_html_contents=cpaBuildGetContents($_server_url);
}
else{
	//Get contents (ignore errors)
	$_html_contents=@cpaBuildGetContents($_server_url);
}

if($_html_contents){
	if($config['mirror'] != "cpabuild.com" || $config['asset_mirror'] != "bucket.cpabuild.com"){

		$_html_contents=str_replace("https://cpabuild.com/",$protocol."://".$config['mirror']."/",$_html_contents);
		$_html_contents=str_replace("https://bucket.cpabuild.com/",$protocol."://".$config['asset_mirror']."/",$_html_contents);

		$_html_contents=str_replace("http://cpabuild.com/",$protocol."://".$config['mirror']."/",$_html_contents);
		$_html_contents=str_replace("http://bucket.cpabuild.com/",$protocol."://".$config['asset_mirror']."/",$_html_contents);

		$_html_contents=str_replace("//bucket.cpabuild.com/",$protocol."://".$config['asset_mirror']."/",$_html_contents);
		$_html_contents=str_replace("//cpabuild.com/",$protocol."://".$config['mirror']."/",$_html_contents);
	}
	echo $_html_contents;
