...
<br><br>
<strong style='font-size: 20px;'>IMPORTANT: To fix, please add public permissions to the folder $_CWD. This can be done in your FTP client, file manager, or terminal.</strong>
<br>
<pre>Example terminal command: sudo chmod 777 $_CWD</pre>
</div>";
...
<p>
	Pretty URL test: <a href='<?php echo $current_url?>cpabuild-deployment-test' target='_blank'><?php echo $current_url?>cpabuild-deployment-test</a>
</p>
<p>
	Non-Pretty URL test: <a href='<?php echo $current_url?>?cpabuild-deployment-test' target='_blank'><?php echo $current_url?>?cpabuild-deployment-test</a> (Notice the added ? question mark)
</p>
...
$template_file_path=$_CWD . "/tmp_ht";
...
if($target_file_exists && is_htaccess_running()) {
	...
	if(!$template_file_exists){
		echo "<div class='status status-error'>Missing required HTACCESS file. Please download the package and try again.</div>";
...
<ol>
	...
	?>
	<li>Start adding URL's to your website here: <a target="_blank" href="https://members.cpabuild.com/privateURL">https://members.cpabuild.com/privateURL</a></li>
	<li>Change your homepage (currently this page). Edit the file <strong>config.php</strong> and change the line <pre>"default"=>"install",</pre> to <pre>"default"=>"my-private-uri",</pre> Where <strong>my-private-uri</strong> is the URI from CPABuild (you can use any private uri as a homepage).</li>
	...
	<li>You can check if your package is up to date by visiting <a href="<?php echo $current_url; ?>version.php" target="_blank"><?php echo $current_url; ?>version.php</a></li>
</ol>
</body>
</html>